...
function getAccentColor(driverName: string): string {
...
function getDriverNumber(driverName: string): number {
  const name = driverName.toUpperCase();
...
export async function GET() {
  try {
    ...
    if (!fs.existsSync(contentsPath)) {
      return NextResponse.json({ error: "CONTENTS directory not found" }, { status: 404 });
    }
    ...
    const driversData = await Promise.all(driverDirs.map(async (dirName) => {
      ...
      // Read MUSICS
      let musics: { url: string, name: string }[] = [];
    ...
    return NextResponse.json({ drivers: driversData });
